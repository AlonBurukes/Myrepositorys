...
void handle_tcp(QList<QStandardItem *> *row, const struct sniff_tcp *tcp, uint16_t size){
	...
	switch(destinationPort){
        ...
        case PORT_FTP_CONTROLL: {
            handle_ftp(row, payload, size - size_tcp);
            break;
        }
        case PORT_FTP_TRANSLATION: {
            handle_ftp(row, payload, size - size_tcp);
            break;
        }
        case PORT_TELNET: {
            handle_telnet(row, payload, size - size_tcp);
            break;
        }
	}
	if(portFound == false){
...
void handle_tcp_fill(QString *infoStr, const struct sniff_tcp *tcp, uint16_t size){
	...
	switch(destinationPort){
        ...
        case PORT_FTP_CONTROLL: {
            handle_ftp_fill(infoStr, payload, size - size_tcp);
            portFound = true;
            break;
        }
        case PORT_FTP_TRANSLATION: {
        ...
        case PORT_TELNET: {
            handle_telnet_fill(infoStr, payload, size - size_tcp);
            break;
        }
	}
